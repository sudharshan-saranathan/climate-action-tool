...
from core.flows.flows import MassFlow, EnergyFlow
from core.flows.parameters import Ratio, TemperatureParam, PressureParam, Factor
from core.flows.dimensions import Energy, Currency, Mass, Power
from core.flows.profiles import ProfileRef, FixedProfile
...
class Fuel(MassFlow):
    ...
    def __init__(self, value: float = 0.0, units: str | None = None):
        ...
        """
        props = {
            "specific_energy": Ratio(
                Energy, Mass, label="specific_energy", is_variable=False
            ),
            "emission_factor": Ratio(
                Mass, Mass, label="emission_factor", is_variable=False
            ),
...
class Electricity(Power):
    ...
    def __init__(self, value: float = 0.0, units: str | None = None):
        ...
        props = {
            ...
            "emissions_factor": Ratio(
                Mass, Power, label="emissions_factor", is_variable=False
            ),
            "variability": Factor(
                "Variability", color="#daa520", icon_name="mdi.sine-wave"
            ),
            "ramp_rate": Factor(
                "Ramp Rate", color="#20b2aa", icon_name="mdi.speedometer"
            ),
        }
        self._props = props
